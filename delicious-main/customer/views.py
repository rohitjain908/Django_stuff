...
def menu(request):
	if request.method == 'POST':
		...
		if Customer.objects.filter(id = cust_id).exists():
			...
			Orderdishes=serializers.serialize("json",Orderdishes)
			Orderdishes = dumps(Orderdishes)
			

			

			return render(request,'menu.html',{'dish' : dish,'customer' : customer, 'rest_id' : rest_id, 'order' : order,'Orderdishes':Orderdishes})
		else:
			customer = None
			return render(request,'menu.html',{'dish' : dish,'customer' : customer, 'rest_id' : rest_id,
			'order':None,'Orderdishes':None})
...
def cart(request):
	...
	context = {'items':items,'order':order,'itemsJSON':itemsJSON}
...
###these all funtions are used for api calls 
...
def post_order(request):
	dish_id= json.loads(request.POST['id'])
	print(dish_id)
	dish = Dish.objects.get(id = dish_id)
	cust_id = request.session["cust_id"]
	customer = Customer.objects.get(id = cust_id)
	if customer.is_login == False :
		return JsonResponse({"message":"customer not login"})
	owner = dish.owner
	restaurant = Resturant.objects.get(owner = owner)

	if Order.objects.filter(customer = customer,restaurant_id = restaurant.id, complete = False,status="placed").exists():
		order = Order.objects.get(customer = customer,restaurant_id = restaurant.id, complete = False)
